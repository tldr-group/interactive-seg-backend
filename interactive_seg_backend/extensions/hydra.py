...
def apply_hydra(
    ...
    fine_crf_params = CRFParams(sxy_g=(0.5, 0.5), sxy_b=(30, 30, 30))
    fine_crf_seg, fine_crf_probs = do_crf_from_probabilites(original_probs, image, n_classes, fine_crf_params)

    coarse_crf_params = CRFParams(sxy_g=(3, 3))
    coarse_crf_seg, coarse_crf_probs = do_crf_from_probabilites(original_probs, image, n_classes, coarse_crf_params)
